...
//Key to our cave: RED, PINK, YELLOW, PINK, RED, YELLOW, Red, GREEN, BLUE
//Key in numbers: 1, 5, 2, 5, 1, 2, 1, 4, 3
/*
1. Red
2. Yellow
3. Blue
...
public class ColorCave extends JPanel implements MouseListener, KeyListener {
    ...
    public ColorCave() {
        ...
        loader.deserialize("CaveData\\M1.ser");  // small maze 1
        navigator = new CaveNavigator(loader);

        startRoom = loader.getStart();
        endRoom = loader.getEnd();

        // Inform the player how to begin the game
        System.out.println("Click on any door to start");
    ...
    public void paintComponent(Graphics g) {
        ...
        if (startTime == 0) {
            ...
            x = (System.currentTimeMillis() - startTime) / 1000.0;
        }
        x = Math.round(x * 100.0) / 100.0;

        if (navigator.getCurrentRoom() == endRoom) {
            // Display end screen
        ...
        } else if (navigator.getCurrentRoom() == startRoom) {
            // Display start page
            g2.setColor(Color.BLACK);
            g2.fillRect(0, 0, frame.getWidth(), frame.getHeight());
            g2.setColor(Color.WHITE);
            ...
            g2.drawString("Click on any door to start", 80, 300);

            Set<Door> doors = navigator.getCurrentRoom().getDoors();
            int startX = 400;
            int startY = 400;
            int doorWidth = 100;
            int doorHeight = 200;
            ...
            g2.setColor(Color.WHITE);
            g2.setFont(new Font("Arial", Font.BOLD, 44));
            g2.drawString("Welcome to Room: " + navigator.getCurrentRoom().getName(), 80, 200);
